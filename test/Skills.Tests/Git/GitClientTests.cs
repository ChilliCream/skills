...
public class GitClientTests
{
    ...
    public async Task CloneAsync_Throws_GitCloneException_For_NonExistent_Local_Path()
    {
        ...
        var fakeRepo = Path.Combine(Path.GetTempPath(), $"skills-fake-{Guid.NewGuid():N}");
        var targetDir = Path.Combine(Path.GetTempPath(), $"skills-test-{Guid.NewGuid():N}");
    ...
    private sealed class FakeGitOnPath : IDisposable
    {
        ...
        {
            _dir = Path.Combine(Path.GetTempPath(), $"skills-fakegit-{Guid.NewGuid():N}");
            Directory.CreateDirectory(_dir);

            var gitPath = Path.Combine(_dir, "git");
            var script = "#!/bin/sh\nprintf '%s' \"$SKILLS_FAKE_GIT_STDERR\" 1>&2\nexit "
            ...
            Environment.SetEnvironmentVariable("SKILLS_FAKE_GIT_STDERR", stderr);
        ...
        public void Dispose()
        {
            ...
            Environment.SetEnvironmentVariable("SKILLS_FAKE_GIT_STDERR", null);
