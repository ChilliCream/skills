...
public class SafePathContainmentTests : IDisposable
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "skills-path-containment-" + Guid.NewGuid().ToString("N"));
    ...
    {
        // Arrange
        var baseDir = Path.GetFullPath(Path.Combine(Path.GetTempPath(), "skills-base"));
    ...
    public void Contains_SiblingDirectoryWithSamePrefix_ReturnsFalse()
    {
        // Arrange
        var baseDir = Path.GetFullPath(Path.Combine(Path.GetTempPath(), "skills-base"));
        var sibling = Path.GetFullPath(Path.Combine(Path.GetTempPath(), "skills-base-other"));
    ...
    public void Contains_LeafSymlinkPointingOutside_ReturnsTrue()
    {
        ...
        // skills-managed agent->canonical symlink work.
        var baseDir = Path.Combine(_root, "base");
        var outside = Path.Combine(_root, "outside");
        Directory.CreateDirectory(baseDir);
        Directory.CreateDirectory(outside);
