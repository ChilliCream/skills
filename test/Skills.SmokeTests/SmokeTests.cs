...
public class SmokeTests
{
    ...
    {
        // Act
        var result = await RunSkillsAsync("--version");
    ...
    {
        // Act
        var result = await RunSkillsAsync("--help");

        // Assert
        Assert.Equal(0, result.ExitCode);
        Assert.Contains("Usage:", result.StandardOutput);
        Assert.Contains("skills", result.StandardOutput);
    ...
    public async Task Init_Creates_SkillMd_In_Named_Directory()
    {
        ...
        {
            // Act
            var result = await RunSkillsAsync("init test-skill", workingDirectory: tempDir);
    ...
    public async Task List_Does_Not_Crash_In_Empty_Directory()
    {
        ...
        {
            // Act
            var result = await RunSkillsAsync("list", workingDirectory: tempDir);
    ...
    private static async Task<BufferedCommandResult> RunSkillsAsync(string arguments, string? workingDirectory = null)
    {
    ...
    {
        var configured = Environment.GetEnvironmentVariable("SKILLS_SMOKE_TEST_CONFIGURATION");
    ...
    {
        var configuredFramework = Environment.GetEnvironmentVariable("SKILLS_SMOKE_TEST_TFM");
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "skills-smoke-" + Path.GetRandomFileName());
