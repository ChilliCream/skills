...
public class AddCommandSnapshotTests : IDisposable
{
    ...
    private const string LocalDisplaySource = "/skills-test/local";
    ...
    private const string CanonicalPath = "/skills-test/.agents/skills/alpha";
    private const string InstallPath = "/skills-test/.claude/skills/alpha";
    ...
    {
        _workspace = Path.Combine(Path.GetTempPath(), "skills-add-snap-" + Guid.NewGuid().ToString("N"));
    ...
    public async Task Add_Without_Source()
    {
        ...
        output.MatchInlineSnapshot(
            ...
            Usage: skills add <source> [options]
            """);
    }
    ...
    public async Task Add_With_Skill_Filter_No_Match()
    {
        ...
        output.MatchInlineSnapshot(
            """
            $ skills add ./local-path --yes --agent claude-code --skill nope
            # exit 1
    ...
    public async Task Add_Yes_Mode_Installs_Skill_Shows_Summary()
    {
        ...
        output.MatchInlineSnapshot(
            ...
            ┌─Installed 1 skill(s)─────────────────────────────────────────────────────────┐
            │ ✓ alpha                                                                      │
            │   → /skills-test/.claude/skills/alpha                                        │
            └──────────────────────────────────────────────────────────────────────────────┘
