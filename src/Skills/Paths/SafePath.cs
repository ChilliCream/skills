...
namespace Skills.Paths;

/// <summary>
/// Centralizes every path-safety decision in skills: containment, real-path
...
internal static class SafePath
{
    ...
    /// skills-managed agent-&gt;canonical leaf link or a stale self-loop is not read as an
    ...
    private static string ContainAndRefuseReparseLeaf(string path, string containRoot, string refusalTitle)
    {
        ...
        if (LeafIsReparsePointOrUnprobeable(resolved))
        {
            throw new CliException(
                ...
                title: refusalTitle,
                hint: "The final path component is a symlink or junction; skills will not read or write through it.");
        }
