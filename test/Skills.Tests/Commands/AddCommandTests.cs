...
public class AddCommandTests : IDisposable
{
    ...
    {
        _workspace = Path.Combine(Path.GetTempPath(), "skills-add-" + Guid.NewGuid().ToString("N"));
    ...
    {
        // Arrange
        var canonical = Path.Combine(_workspace, ".skills", "skills", "alpha");
        ...
        var services = BuildServices(
            ...
            configureInstaller: i =>
            {
                i.OnGetCanonicalPath = (skill, _, _) => Path.Combine(_workspace, ".skills", "skills", skill);
    ...
    public async Task Add_Yes_Warns_About_Overwrite_Before_Installing()
    {
        ...
        installer.OnGetCanonicalPath = (skill, _, _) => Path.Combine(_workspace, ".skills", "skills", skill);
