    ...
    CliExecutionContext executionContext) : BaseCommand("update", "Check for skill updates.")
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (lockFile.Skills.Count == 0)
        {
            if (skillFilter is null)
            {
                ...
                interaction.WriteDim($"Install skills with: {executionContext.CommandName} add <package> -g");
            }
            return (0, 0, 0);
        }
        ...
        if (updates.Count > 0)
        {
            ...
            foreach (var (name, _) in updates)
            {
                ...
                interaction.WriteDim(
                    $"  Run: {executionContext.CommandName} add {BuildInstallSource(lockFile.Skills[name])} -g -y");
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (projectSkills.Count == 0)
        {
            if (skillFilter is null)
            {
                ...
                interaction.WriteDim($"Install project skills with: {executionContext.CommandName} add <package>");
            }
            return (0, 0);
        }
    ...
    private void PrintSkippedSkills(IReadOnlyList<SkippedSkill> skipped)
    {
        ...
        foreach (var skill in skipped)
        {
            ...
            interaction.WriteDim($"    To update: {executionContext.CommandName} add {manual} -g -y");
        }
    }
    ...
    private void PrintLegacyProjectSkills(IReadOnlyList<(string Name, LocalSkillLockEntry Entry)> legacy)
    {
        ...
        foreach (var (name, entry) in legacy)
        {
            ...
            interaction.WriteMarkupLine($"  [grey85]*[/] {Markup.Escape(name)}");
            interaction.WriteDim($"    To refresh: {executionContext.CommandName} add {reinstall} -y");
        }
    }
