    ...
    AgentEnvironment agentEnvironment)
{
    ...
    public void ShowBanner()
    {
        ...
        var commandName = Markup.Escape(context.CommandName);
        WriteCommandTable(
            ($"[dim]$[/] [grey78]{commandName} add[/] [dim]<package>[/]", "Add a new skill"),
            ($"[dim]$[/] [grey78]{commandName} remove[/]", "Remove installed skills"),
            ($"[dim]$[/] [grey78]{commandName} list[/]", "List installed skills"),
            ($"[dim]$[/] [grey78]{commandName} update[/]", "Update installed skills"),
            ($"[dim]$[/] [grey78]{commandName} init[/] [dim][[name]][/]", "Create a new skill"));

        interaction.WriteLine();
        interaction.WriteMarkupLine(
            $"[dim]try:[/] [grey78]dnx {commandName} add[/] [#fb522e]chillicream/agent-skills[/]");
        interaction.WriteLine();
    }
    ...
    public void ShowCuratedHelp()
    {
        interaction.WriteLine();
        var commandName = Markup.Escape(context.CommandName);
        interaction.WriteMarkupLine($"[bold]Usage:[/] [grey78]{commandName}[/] [dim]<command>[/] [dim][[options]][/]");
        interaction.WriteLine();
        ...
        WriteCommandTable(
            ($"[dim]$[/] [grey78]{commandName} add[/] [dim]chillicream/agent-skills[/]", ""),
            ($"[dim]$[/] [grey78]{commandName} add[/] [dim]owner/repo@skill-name -y[/]", ""),
            ($"[dim]$[/] [grey78]{commandName} list[/] [dim]-g --json[/]", ""),
            ($"[dim]$[/] [grey78]{commandName} update -y[/]", ""),
            ($"[dim]$[/] [grey78]{commandName} init[/] [dim]my-skill[/]", ""));
        interaction.WriteLine();

        interaction.WriteDim($"Use '{context.CommandName} <command> --help' for detailed command-specific help.");
        interaction.WriteLine();
    }
