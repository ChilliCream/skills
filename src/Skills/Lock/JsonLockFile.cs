...
internal abstract class JsonLockFile<TFile> where TFile : class
{
        ...
        try
        {
            ...
            if (version > LatestVersion && !throwOnCorrupt)
            {
                Console.Error.WriteLine(
                    $"Warning: Lock file was written by a newer version of skills (v{version}, this is v{LatestVersion}). Data will be preserved but some fields may be ignored.");
            }
    ...
    private CliException NewerVersionException(string lockPath, int version)
        => new(
            ExitCodeConstants.Failure,
            $"Refusing to modify {Noun} '{lockPath}': on-disk version v{version} is newer than this skills supports (v{LatestVersion}).");
