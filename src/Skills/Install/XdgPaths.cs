...
internal sealed class XdgPaths(ISystemEnvironment system)
{
    ...
    /// <c>&lt;data-home&gt;/skills</c> root, so the lock always describes where skills actually live.
    /// </summary>
    ...
    {
        return Path.Combine(GetDataHome(), "skills");
    }

    public string GetConfigDirectory()
    {
        return Path.Combine(GetConfigHome(), "skills");
    }

    public string GetLogDirectory()
    {
        return Path.Combine(GetStateHome(), "skills", "logs");
    }
