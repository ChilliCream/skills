...
public class SkillDiscoveryTests : IDisposable
{
    ...
    {
        _testDir = Path.Combine(Path.GetTempPath(), $"skills-discovery-test-{Guid.NewGuid():N}");
        _outsideDir = Path.Combine(Path.GetTempPath(), $"skills-discovery-outside-{Guid.NewGuid():N}");
