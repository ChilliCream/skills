...
namespace Skills.Tests.Plugins;
...
public class PluginManifestTests : IDisposable
{
    ...
    {
        _testDir = Path.Combine(Path.GetTempPath(), $"skills-manifest-test-{Guid.NewGuid():N}");
        _outsideDir = Path.Combine(Path.GetTempPath(), $"skills-manifest-outside-{Guid.NewGuid():N}");
    ...
    public async Task ReadPlugins_Should_RejectMarketplaceEntry_When_PluginBaseResolvesOutsideBaseViaSymlink()
    {
        ...
        var outsideDir = Path.Combine(Path.GetTempPath(), $"skills-outside-{Guid.NewGuid():N}");
